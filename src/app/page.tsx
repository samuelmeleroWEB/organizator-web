...
export default function Home() {
  ...
  return (
    <div className="h-auto md:h-screen md:overflow-hidden flex flex-col bg-slate-50 text-slate-800 p-4 sm:p-4 lg:p-6 relative selection:bg-blue-200">
      ...
      <main className="max-w-7xl w-full mx-auto relative z-10 flex flex-col flex-1">
        ...
        <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-start h-full overflow-hidden">
          
          {/* Left Panel: Inputs */}
          <div className="lg:col-span-5 flex flex-col gap-6 lg:pr-2 pb-2 h-full overflow-y-auto">
          ...
          <div className={`lg:col-span-7 flex-col relative h-full ${plan || loading ? 'flex' : 'hidden md:flex'}`}>
            <PlanResult plan={plan} loading={loading} />
          </div>
        </div>

        <footer className="text-xs text-gray-300 text-center py-2 mt-auto">
          Powered by GPT-4o
        </footer>
      </main>
    </div>
